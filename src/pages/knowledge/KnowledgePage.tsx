...
import { ChatTurnView } from "@/features/knowledge/ChatTurn";
import { SEEDED_THREAD, SUGGESTED_PROMPTS, RECENT_THREADS } from "@/mocks/seed/knowledge";
import type { ChatTurn } from "@/mocks/seed/knowledge";
  ...
  const send = (text: string) => {
    ...
    setTurns((t) => [
      ...
      { id: `u-${Date.now()}`, role: "user", text },
      {
        id: `a-${Date.now()}`,
        role: "assistant",
        text: "Searching across 1,000 meetings, 2,000 actions, and 600 decisions…",
      },
    ]);
    setDraft("");
  };
      ...
      <aside className="hidden md:flex w-[260px] shrink-0 flex-col border-r border-border-soft bg-surface">
        ...
        <div className="p-3 border-t border-border-soft space-y-1.5">
          <Capability
            ...
            label={`${TENANT.totalMeetings.toLocaleString()} meetings · ${TENANT.totalDecisions.toLocaleString()} decisions`}
          />
          <Capability
            ...
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                <h1 className="font-display text-h3 text-ink-primary tracking-tight truncate">
                  Q3 enterprise rollout decisions
                </h1>
                <p className="text-[11px] text-ink-muted">
                  Knowledge memory · grounded in your meeting graph, decisions, and action items
                </p>
              </div>
            </div>
          ...
          <div className="max-w-3xl mx-auto space-y-3">
            <div className="ai-edge rounded-xl bg-surface p-2 flex items-end gap-2 shadow-elev-1">
              ...
              <textarea
                ...
                placeholder="Ask anything across your meetings — “Why did we decide to phase the rollout?”"
                rows={1}
            ...
            <p className="text-[11px] text-ink-muted text-center">
              Knowledge memory is permission-aware — answers respect each member's department-level access.
