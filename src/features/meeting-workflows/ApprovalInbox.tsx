...
import type { ApprovalItem } from "@/mocks/seed/meetingWorkflows";
      ...
      {items.map((item) => (
        <li key={item.id} className="p-4 hover:bg-subtle/40 transition-colors group">
          <div className="flex items-start gap-3">
            <div className="h-9 w-9 shrink-0 rounded-md grid place-items-center bg-gradient-to-br from-brand-500/15 to-accent-500/20 text-accent-700">
            ...
            <div className="flex-1 min-w-0">
              ...
              <h4 className="mt-1 text-body font-medium text-ink-primary leading-snug">{item.title}</h4>
              ...
              <div className="mt-2 flex items-center gap-2 flex-wrap text-[12px]">
                {item.meetingRef && (
                  <span className="text-ink-secondary bg-subtle px-1.5 py-0.5 rounded">
                    {item.meetingRef}
                  </span>
                )}
                {item.ownerHint && (
