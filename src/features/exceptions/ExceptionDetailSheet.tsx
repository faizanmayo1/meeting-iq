...
import { Sparkles, Check, Clock, DollarSign, MessageSquare, ArrowRight } from "lucide-react";
...
type Props = { exception: ExceptionRow | null; open: boolean; onOpenChange: (v: boolean) => void };

export function ExceptionDetailSheet({ exception, open, onOpenChange }: Props) {
  const [selected, setSelected] = useState<string | null>(null);
  if (!exception) return null;

  const recs = exception.resolutions ?? [];
      ...
      <SheetContent side="right" width="sm:max-w-[600px]" className="flex flex-col p-0">
        <SheetHeader className="space-y-2">
          <div className="flex items-center gap-2 flex-wrap">
            <Badge tone={SEV_TONE[exception.severity]} dot size="sm">
              {exception.severity}
            </Badge>
            {exception.dollarImpact != null && (
              <span className="inline-flex items-center gap-1 text-[11px] font-medium text-ink-secondary tabular-nums">
                <DollarSign className="h-3 w-3" /> ${exception.dollarImpact.toLocaleString()} at stake
              </span>
            )}
            {exception.etaImpact && exception.etaImpact !== "—" && (
              <span className="inline-flex items-center gap-1 text-[11px] font-medium text-warning-700 tabular-nums">
                <Clock className="h-3 w-3" /> {exception.etaImpact}
              </span>
            )}
          </div>
          <SheetTitle className="!text-h2 leading-tight pr-8">{exception.title}</SheetTitle>
          <p className="text-body-sm text-ink-muted">
            {exception.load !== "—" && (
              <>
                Load <span className="font-mono text-ink-primary">{exception.load}</span>
                {" · "}
              </>
            )}
            {exception.unit && (
              <>
                Unit <span className="font-mono text-ink-primary">{exception.unit}</span>
                {" · "}
              </>
            )}
            {exception.driver !== "—" && <>{exception.driver}</>}
            <span className="mx-1.5">·</span>
            opened {exception.age} ago
            {exception.customer && (
              <>
                <span className="mx-1.5">·</span>
                {exception.customer}
        ...
        <SheetBody className="flex-1 space-y-5">
          ...
          <section className="rounded-lg border border-border-soft bg-subtle/40 p-4">
            <p className="text-caption uppercase tracking-[0.1em] text-ink-muted mb-1.5">Why this fired</p>
            <p className="text-body text-ink-secondary leading-relaxed">{exception.detail}</p>
          </section>
          ...
          {recs.length > 0 ? (
            ...
            <section className="rounded-lg border border-border-soft p-4">
              ...
              <p className="text-body text-ink-secondary">{exception.resolution}</p>
            </section>
          )}
          ...
          <section>
            ...
            <ol className="relative space-y-3">
              ...
              <Activity dot="bg-accent-500" who="FreightIQ Agent" time={`${exception.age} ago`}>
                Detected {exception.kind.replace("_", " ")} signal · created exception · ranked resolutions
              </Activity>
              <Activity dot="bg-brand-500" who="System" time={`${Math.max(1, exception.ageMinutes - 1)}m ago`}>
                Notified <span className="text-ink-primary font-medium">Stefan Trifan</span> via push + Slack #ops-room
              </Activity>
              {exception.state !== "open" && (
                <Activity dot="bg-success-500" who="Stefan Trifan" time="2m ago">
                  Reviewed exception · marked in progress
                </Activity>
              )}
            </ol>
          </section>
